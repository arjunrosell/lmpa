...
<x-page-title>Add New Sales</x-page-title>
@include('layouts.admin.sidebar.navigation-menu')
<x-forms.container>
    <x-forms.title>Add New Sales</x-forms.title>
    <x-forms.divider />
    <x-forms.form method="POST" action="{{ route('admin.sales.store') }}">
        @csrf
        <div class="mb-4 grid gap-4 sm:grid-cols-1 sm:gap-6">
            ...
            <x-forms.input type="date" label="Date" name="sale_date" />
            <x-forms.input
                type="number"
                label="Quantity"
                name="quantity"
